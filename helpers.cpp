...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
  	...
  	//Traversing the grid to calculate probability sum
  	...
  	//Loop to calculate the normalized probability
  	for(int r=0; r<height; r++) {
    	for(int c=0; c<width; c++) {
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// your code here
  	...
  	//Variables to calculate the blurring point
  	float center_probability = 1.0 - blurring;
  	float corner_probability = blurring/12.0;
  	float adjacent_probability = blurring/6.0;
  	
  	//Unnormalized matrix
  	vector <vector <float>> window = {
      {corner_probability, adjacent_probability, corner_probability},
      {adjacent_probability, center_probability, adjacent_probability},
      {corner_probability, adjacent_probability, corner_probability}
    };
        	...
        	float gridVal = grid[i][j];
          	for(int dx=-1; dx<2; dx++) {
            	for(int dy=-1; dy<2; dy++) {
                	int newI = (i+dy+height) % height;
                  	int newJ = (i+dx+width) % width;
                  	float mult = window[dx+1][dy+1];
                  	newGrid[newI][newJ] += gridVal * mult;
                }
            }
        }
    }
